...
export const getCartItems = (key?: string) => {
  const storedItems = localStorage.getItem(key || "ecommerceCartNext");
  if (storedItems) {
    return JSON.parse(storedItems);
  } else {
    localStorage.setItem("ecommerceCartNext", "[]");
    return [];
  }
};
...
export const updateVanillaCartItemQuantityToLs = (
  ...
  localStorage.setItem("ecommerceCartNext", JSON.stringify(updatedCart));
};
